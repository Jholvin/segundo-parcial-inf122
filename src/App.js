...
function App() {
  ...
  const eliminarMeta = (id) => {
    const metaEliminada = metas.find((meta) => meta.id === id);
  
    setMetas((prevMetas) => {
      const nuevasMetas = prevMetas.filter((meta) => meta.id !== id);
  
      if (metaEliminada.completada) {
        setCompletadas((prevCompletadas) => prevCompletadas - 1);
      } else {
        setPendientes((prevPendientes) => prevPendientes - 1);
      }
  ...
  const marcarComoCompletada = (id) => {
    setMetas((prevMetas) => {
      const nuevasMetas = prevMetas.map((meta) =>
        meta.id === id ? { ...meta, completada: !meta.completada } : meta
      );
  
      const metaActualizada = nuevasMetas.find((meta) => meta.id === id);
  
      if (metaActualizada.completada) {
        setCompletadas((prevCompletadas) => prevCompletadas + 1);
        setPendientes((prevPendientes) => prevPendientes - 1);
      } else {
        setCompletadas((prevCompletadas) => prevCompletadas - 1);
  ...
  return (
    <div className="App">
      <h1 className='titlee'>Mis metas</h1>
      ...
      <div className='COMPEN'>
        <p className='COM'>Completadas: {completadas}</p>
        <p className='PEN'>Pendientes: {pendientes}</p>
      </div>
      ...
      <center><ul>
        {metas.map((meta) => (
          <center><li
            className='listilla'
            key={meta.id}
            style={{ textDecoration: meta.completada ? 'line-through' : 'none' }}
          >
            {meta.texto}
            <div className='tick'>
              <svg
              ...
              <svg
                ...
                viewBox="0 0 20 21"
                fill="none"
                onClick={() => marcarComoCompletada(meta.id)}
                className="tick-icon"
              >
